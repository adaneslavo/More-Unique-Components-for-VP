...
function GarrisonRangePower(iPlayer, iUnit, iX, iY)
	...
	if not(pPlayer and pPlayer:GetCivilizationType() == GameInfoTypes.CIVILIZATION_POLAND) then 
		return 
	end
	...
	if pCity ~= nil 
		then bHasBarbican = pCity:IsHasBuilding(GameInfoTypes.BUILDING_POLAND_BARBICAN) 
	end
